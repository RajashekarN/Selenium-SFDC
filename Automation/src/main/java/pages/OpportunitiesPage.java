...
public class OpportunitiesPage extends ReusableLibrary {
	...
	@FindBy(xpath = "//*[contains(@id,''duel_select_0')]")
...
/*	@FindBy(xpath = "//*[text()='Sales Stage']/parent::li/p[2]/span")
	...
	public void associateProperty() {
		...
		Utility_Functions.timeWait(2);
		List<WebElement> propertyList = driver.findElements(By.xpath("//table[@class='slds-table slds-table--bordered slds-table--cell-buffer slds-table--fixed-layout cPropertySearch']//th[@class='slds-truncate']/a"));
		Utility_Functions.timeWait(1);
		...
		if(saveButtonProperty.isDisplayed()) {
			...
			report.updateTestLog("Verify Opportunity",
					"Associate property to an opportunity successfully ",
					Status.PASS);
		} else {
			report.updateTestLog("Verify Opportunity",
					"Associat property to an opportunity failed ",
					Status.FAIL);
		} 	
	}
}